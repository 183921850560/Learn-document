...
class Status(unittest.TestCase):
    ...
    def test_RescueInfo(self):
        ...
        #传入给智能屏下发信息的uri
        self.sta_uri = 'http://122.112.212.213:8777/api/v1/screen/status?access_token='
        self.status_url = self.sta_uri + self.token_value
        ...
        #将传入的参数转换成json格式
        self.status_value = json.dumps(self.status_data)

        #请求给智能屏下发基本信息
        self.status_req = requests.post(headers=self.header, url=self.status_url, data=self.status_value)
        print('Code status:', self.status_req.status_code, self.status_req.json())
