...
#将变量提取出来
sheet = common.get_xls_sheet("checkUserCodeExists.xls", "checkUserCodeExists")
...
class checkUserCodeExists(unittest.TestCase):
    def setParameters(self, case_name, method, url,userCode, operatorUserId, msg):
        self.case_name = str(case_name)
        self.method = str(method)
        self.url = str(url)
        self.param = [str(userCode), str(operatorUserId)]
        self.msg = int(msg)
        self.response = None
        self.info = None
    ...
    #操作步骤（创建用户）
    def testcheckUserCodeExists(self):
...
if __name__ == "__main__":
    ...
    suite.addTest(checkUserCodeExists("testcheckUserCodeExists"))
