...
#将变量提取出来
sheet = common.get_xls_sheet("deleteUsers.xls", "deleteUsers")
...
class deleteUsers(unittest.TestCase):
    def setParameters(self, case_name, method, url,userGuids, operatorUserId, msg):
        self.case_name = str(case_name)
        self.method = str(method)
        self.url = str(url)
        self.guid_user = str(eval(userGuids))
        self.param = [eval(userGuids), int(operatorUserId)]
        self.msg = int(msg)
        self.response = None
        self.info = None
...
if __name__ == "__main__":
    ...
    suite.addTest(deleteUsers("testdeleteUsers"))
