...
#将变量提取出来
sheet = common.get_xls_sheet("queryUser.xls", "queryUser")
...
class queryUser(unittest.TestCase):
    def setParameters(self, case_name, method, url,pageNum, pageSize, operatorUserId, msg):
        self.case_name = str(case_name)
        self.method = str(method)
        self.url = str(url)
        self.param = [int(pageNum), int(pageSize), int(operatorUserId), int(msg)]
        self.msg = int(msg)
        self.response = None
        self.info = None
...
if __name__ == "__main__":
    ...
    suite.addTest(queryUser("testqueryUser"))
