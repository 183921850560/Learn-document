...
#将变量提取出来
sheet = common.get_xls_sheet("fetchUsersByGuids.xls", "fetchUsersByGuids")
...
class fetchUsersByGuids(unittest.TestCase):
    def setParameters(self, case_name, method, url,userGuids, msg):
        self.case_name = str(case_name)
        self.method = str(method)
        self.url = str(url)
        self.param = [eval(userGuids)]
        self.msg = int(msg)
        self.response = None
        self.info = None
    ...
    #操作步骤（创建用户）
    def testfetchUsersByGuids(self):
...
if __name__ == "__main__":
    ...
    suite.addTest(fetchUsersByGuids("testfetchUsersByGuids"))
