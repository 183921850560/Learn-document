...
#将变量提取出来
sheet = common.get_xls_sheet("authUserLift.xls", "authUserLift")
...
class authUserLift(unittest.TestCase):
    def setParameters(self, case_name, method, url,dists, operatorUserId, userGuid, type, userGroupGuid, msg):
        self.case_name = str(case_name)
        self.method = str(method)
        self.url = str(url)
        self.param = [eval(dists), int(operatorUserId), int(userGuid), str(type), int(userGroupGuid)]
        self.msg = int(msg)
        self.response = None
        self.info = None
...
if __name__ == "__main__":
    ...
    suite.addTest(authUserLift("testauthUserLift"))
