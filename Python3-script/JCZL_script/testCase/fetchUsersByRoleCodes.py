...
#将变量提取出来
sheet = common.get_xls_sheet("fetchUsersByRoleCodes.xls", "fetchUsersByRoleCodes")
...
class fetchUsersByRoleCodes(unittest.TestCase):
    def setParameters(self, case_name, method, url,roleCodes, msg):
        self.case_name = str(case_name)
        self.method = str(method)
        self.url = str(url)
        self.param = [str(roleCodes)]
        self.msg = int(msg)
        self.response = None
        self.info = None
    ...
    #操作步骤（创建用户）
    def testfetchUsersByRoleCodes(self):
...
if __name__ == "__main__":
    ...
    suite.addTest(fetchUsersByRoleCodes("testfetchUsersByRoleCodes"))
